...
export default function VerificationForm() {
  ...
  async function requestCode() {
    ...
    try {
      ...
      setSent(true); setNotice(payload.data?.debugCode ? `Development code: ${payload.data.debugCode}` : "A new verification code was sent to your email.");
  ...
  async function verifyCode(event: FormEvent) {
    ...
    try {
      const response = await fetch(clientApiUrl("/auth/client/verify-code"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, channel: "email", code: cleanCode }) });
      const payload = await response.json();
  ...
  return <main className="min-h-screen bg-canvas text-ink"><div className="m-stripe" aria-hidden="true" /><section className="border-b border-line bg-surface text-ink"><div className="mx-auto max-w-[1000px] px-5 py-16 sm:px-8 sm:py-24"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-action">Tauqeer Mustafa Inc. — Secure onboarding</p><h1 className="mt-5 max-w-3xl text-[clamp(3rem,7vw,5.5rem)] font-bold uppercase leading-[0.94] tracking-[-0.04em]">Verify your<br /><span className="text-action">email address.</span></h1><p className="mt-7 max-w-xl text-base font-light leading-7 text-ink-muted">Enter the verification code sent to your email to activate your Tauqeer Mustafa Inc. project workspace.</p></div></section><div className="mx-auto max-w-[700px] px-5 py-12 sm:px-8 sm:py-20"><section className="border border-line-2 bg-surface p-6 sm:p-8"><div className="flex items-center gap-3"><Mail className="h-5 w-5 text-action" aria-hidden /><h2 className="text-2xl font-bold uppercase">Email verification</h2>{verified ? <CheckCircle2 className="ml-auto h-5 w-5 text-[#26733b]" aria-hidden /> : null}</div><p className="mt-3 text-sm font-light leading-6 text-ink-muted">{verified ? "Your email is verified and your workspace is ready." : "Enter the six-digit code from your inbox."}</p>{!verified ? <form className="mt-8" onSubmit={verifyCode}><div className="flex flex-col gap-3 sm:flex-row"><input value={emailCode} onChange={(event) => setEmailCode(event.target.value)} inputMode="numeric" pattern="[0-9]{6}" maxLength={6} placeholder="000000" className="h-12 min-w-0 flex-1 border border-line-2 bg-white px-4 font-mono text-lg tracking-[0.2em] outline-none focus:border-action" required /><button disabled={loading || !emailCode.trim()} type="submit" className="inline-flex min-h-12 items-center justify-center gap-2 bg-ink px-5 font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-canvas disabled:opacity-40 hover:bg-action">Verify code <ArrowRight className="h-3.5 w-3.5" aria-hidden /></button></div><button type="button" onClick={requestCode} disabled={loading} className="mt-4 inline-flex min-h-10 items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-action"><RefreshCw className="h-3.5 w-3.5" aria-hidden /> Resend email code</button></form> : <Link href="/client/login" className="mt-7 inline-flex min-h-12 items-center gap-2 bg-ink px-5 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-canvas transition hover:bg-action">Continue to sign in <ArrowRight className="h-3.5 w-3.5" aria-hidden /></Link>}</section>{notice ? <p className="mt-6 border border-[#b9d5c2] bg-[#f2f8f3] p-4 text-sm text-[#26733b]" role="status">{notice}</p> : null}{error ? <p className="mt-6 border border-[#efb8b2] bg-[#fff5f4] p-4 text-sm text-[#a52c21]" role="alert">{error}</p> : null}</div></main>;
}
