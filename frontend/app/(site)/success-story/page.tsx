...
export default function SuccessStoryPage() {
  return (
    <>
      ...
      {/* ── Key stats banner ── */}
      <Section className="bg-[#0A1628]" labelledBy="success-stats">
        <h2 id="success-stats" className="sr-only">Company statistics</h2>
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-4 lg:grid-cols-8">
          {keyStats.map((stat) => (
            <div key={stat.label} className="flex flex-col items-center text-center">
              <stat.icon className="h-5 w-5 text-[#7FA8FF]" aria-hidden />
      ...
      <Section className="bg-white" labelledBy="story-intro">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2
            id="story-intro"
            className="mt-4 text-balance text-3xl font-semibold leading-tight text-[#0A1628] sm:text-4xl"
          >
            A year-by-year account of how we grew.
          </h2>
          <p className="mt-5 text-base leading-7 text-[#5F6673] sm:text-lg">
      ...
      {/* ── Timeline ── */}
      <Section className="bg-[#F8FAFC]" labelledBy="success-timeline">
        <h2 id="success-timeline" className="sr-only">Timeline</h2>

        <div className="relative">
          {/* Centre spine — visible on lg only */}
          <div
            className="absolute left-1/2 top-0 hidden h-full w-px -translate-x-1/2 bg-[#D7DEE8] lg:block"
            aria-hidden
          />
            ...
            {milestones.map((m) => (
              ...
              >
                ...
                <div
                  ...
                  aria-hidden
                >
                  <span className="inline-flex items-center border border-[#0B5FFF] bg-white px-3 py-1 font-mono text-[11px] font-semibold uppercase tracking-widest text-[#0B5FFF]">
                    {m.year}
                  </span>
                </div>
                ...
                <div className={m.side === "right" ? "lg:pl-12" : "lg:pr-12"}>
                  <article className="border border-[#D7DEE8] bg-white p-7 shadow-[0_1px_2px_rgba(17,24,39,0.04),0_18px_48px_rgba(17,24,39,0.05)]">
                    {/* Mobile year badge */}
                    <span className="mb-4 inline-flex items-center border border-[#0B5FFF] px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-widest text-[#0B5FFF] lg:hidden">
                      {m.year}
                    </span>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <h3 className="mt-1 text-xl font-semibold text-[#0A1628]">{m.title}</h3>
                      </div>
                    </div>

                    <p className="mt-5 text-sm leading-7 text-[#5F6673]">{m.body}</p>
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {m.tags.map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center border border-[#E5E7EB] bg-[#F8FAFC] px-2.5 py-1 text-xs font-medium text-[#374151]"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                >
                  <div className="border-l-4 border-[#0B5FFF] bg-white p-6 shadow-[0_1px_2px_rgba(17,24,39,0.04),0_8px_24px_rgba(17,24,39,0.06)]">
                    <p className="font-mono text-5xl font-semibold leading-none tracking-tight text-[#0A1628]">
                      {m.stat.value}
                    </p>
                    <p className="mt-2 text-sm text-[#5F6673]">{m.stat.label}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ── What made it work ── */}
      <Section className="bg-white" labelledBy="success-principles">
        <SectionHeader
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {principles.map((p) => (
            <Card key={p.title}>
              ...
              <h3 className="mt-6 text-xl font-semibold text-[#0A1628]">{p.title}</h3>
              <p className="mt-4 text-base leading-7 text-[#6B7280]">{p.description}</p>
            </Card>
          ))}
        </div>
      </Section>
      ...
      {/* ── Image break ── */}
      <Section className="bg-[#F8FAFC]" labelledBy="success-image">
      ...
      {/* ── CTA ── */}
      <Section className="bg-[#0A1628]" labelledBy="success-cta">
        <div className="mx-auto max-w-3xl text-center">
          <p className="font-mono text-[11px] font-semibold uppercase tracking-widest text-[#7FA8FF]">
            What's next
          </p>
          <h2
            id="success-cta"
