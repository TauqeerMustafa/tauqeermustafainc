                ...
                return (
                  <tr key={user.id} className="border-b last:border-b-0" style={{ borderColor: "var(--adm-border)" }}>
                    ...
                    <td className="relative px-4 py-4 text-right">
                      <button type="button" aria-label={`Actions for ${user.name}`} onClick={(event) => { event.stopPropagation(); setMenuId(menuId === user.id ? null : user.id); }} className="inline-flex h-9 w-9 items-center justify-center border transition hover:bg-gray-50" style={{ borderColor: "var(--adm-border)", color: "var(--adm-text-2)" }}><MoreHorizontal size={17} /></button>
                      {menuId === user.id ? (
                        <div className="absolute right-4 top-14 z-20 w-44 border p-1 text-left shadow-lg" style={{ borderColor: "var(--adm-border)", background: "var(--adm-surface)" }} onClick={(event) => event.stopPropagation()}>
                          {user.status === "pending" ? <button type="button" onClick={() => changeStatus(user, "approved")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50" style={{ color: "var(--adm-green)" }}><Check size={14} />Approve</button> : null}
                          {user.status === "approved" ? <button type="button" onClick={() => changeStatus(user, "suspended")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50" style={{ color: "var(--adm-red)" }}><PauseCircle size={14} />Suspend access</button> : null}
                          {user.status === "suspended" ? <button type="button" onClick={() => changeStatus(user, "approved")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50" style={{ color: "var(--adm-green)" }}><Check size={14} />Restore access</button> : null}
                          {user.status === "pending" ? <button type="button" onClick={() => changeStatus(user, "rejected")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50" style={{ color: "var(--adm-text-2)" }}><X size={14} />Reject request</button> : null}
                        </div>
                      ) : null}
                    </td>
                  </tr>
