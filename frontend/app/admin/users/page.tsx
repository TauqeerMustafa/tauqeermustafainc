                    ...
                    <td className="relative px-4 py-4 text-right">
                      ...
                      {menuId === user.id ? (
                        <div className="absolute right-4 top-14 z-20 w-44 border p-1 text-left" style={{ borderColor: "var(--adm-border)", background: "var(--adm-surface)" }} onClick={(event) => event.stopPropagation()}>
                          {user.status === "pending" ? <button type="button" onClick={() => changeStatus(user, "approved")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-[var(--adm-surface-2)]" style={{ color: "var(--adm-green)" }}><Check size={14} />Approve</button> : null}
                          {user.status === "approved" ? <button type="button" onClick={() => changeStatus(user, "suspended")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-[var(--adm-surface-2)]" style={{ color: "var(--adm-red)" }}><PauseCircle size={14} />Suspend access</button> : null}
                          {user.status === "suspended" ? <button type="button" onClick={() => changeStatus(user, "approved")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-[var(--adm-surface-2)]" style={{ color: "var(--adm-green)" }}><Check size={14} />Restore access</button> : null}
                          {user.status === "pending" ? <button type="button" onClick={() => changeStatus(user, "rejected")} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-[var(--adm-surface-2)]" style={{ color: "var(--adm-text-2)" }}><X size={14} />Reject request</button> : null}
                        </div>
                      ) : null}
                    </td>
                  </tr>
