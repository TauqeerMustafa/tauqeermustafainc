 ...
 *   WHATSAPP_TOKEN             â€“ permanent system-user or temp access token
 *   WHATSAPP_PHONE_NUMBER_ID   â€“ the default Phone Number ID to send from
 *   WHATSAPP_PHONE_NUMBER_ID_2 â€“ a second number on the same WABA (optional)
 *
 * Supported body.type: text | buttons | list | template | meta_template | media | reaction
 *
 * `list` sends an interactive list â€” up to ten choices in one tap, which is how
...
/** Normalize to international digits only â€” Meta rejects "+" prefix */
function toE164(raw: string): string {
  return raw.replace(/[^0-9]/g, "");
}

async function graphPost(phoneNumberId: string, token: string, body: object) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {
      ...
      listButton,
      listTitle,
      /** Send a step of the scripted lead flow verbatim â€” see lib/wa-flow. */
      flowStep: flowStepId,
    } = body;

    // The sender is chosen by the caller but validated here â€” see lib/wa-numbers.
    ...
    switch (type) {
      ...
      case "buttons": {
        ...
        // something â€” the webhook branches on the id, never on the title.
      ...
      case "list": {
        ...
        // â€” that is the wording customers get from the webhook, so a test send
    ...
    if (messageId) {
      ...
      // Persist directly to the store â€” going through /api/whatsapp/messages
    ...
    if (markReadMessageId) {
      await graphPost(phoneNumberId, token, {
        messaging_product: "whatsapp",
        status: "read",
