...
// Storage keys
//
// The `*Seed` keys hold a fingerprint of the default set this code last wrote.
// They are how an upgrade tells "nobody has touched the seeded copy, so replacing
// it is safe" apart from "an admin wrote this wording deliberately, leave it".
// Without them the only signal available is a heuristic on the copy itself, which
// stops working the moment two successive seeds look alike.
//
// Both are stored as `{ print: string }`, not as the bare string: this client
// passes a string argument through unchanged but JSON.parses every read, so a
// bare JSON print would come back as an array and never compare equal. See
// SeedPrint in lib/wa-store.
export const KEYS = {
